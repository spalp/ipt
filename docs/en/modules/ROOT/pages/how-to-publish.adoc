...
. Export as a .csv or .txt with a UTF-8 encoding and send it for a quality check the [GBIF data validation tool](https://www.gbif.org/tools/data-validator) (recommended but not necessary). Explore and decide whether to address the [reported issues and flags](https://data-blog.gbif.org/post/issues-and-flags/).
