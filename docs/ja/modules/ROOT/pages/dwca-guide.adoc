...
. 行き詰まった場合は、GBIFヘルプデスク（helpdesk@gbif.org）に連絡してください。
...
上級ユーザーは、既存のシステムにGBIF APIを組み込むことでデータセットを一括して登録する許可を得ることができます。link:https://github.com/gbif/registry/tree/master/registry-examples/src/test/scripts[基本的な例]は、Linux（Bash）シェルスクリプトとして提供されています。
