...
NOTE: Red Hat 8、CentOS 8 Stream、Rocky Linux 8などのパッケージは現在開発中です。こちらの link:https://github.com/gbif/ipt/issues/1646[問題] を見てテストにご協力いただくか、代わりにRed Hat 7やCentOS 7をご利用ください。

Red Hat 7やCentOS 7などにIPTをインストールするには、まずGBIFパッケージリポジトリを追加して、IPTパッケージをインストールします。
...
IPTのデプロイに使用される最も一般的なサーブレットコンテナはTomcat、Jetty、Wildfly8（JBoss）です。

=== Tomcat

IPTはTomcat 8.5や9の最新のリリースでテストされ、正しく動作しています。Apache Tomcatのドキュメントは、http://tomcat.apache.org/ に掲載されています。

. Tomcatのインストール - Tomcatのインストールガイド（xref:tomcat-installation-linux.adoc[Linux]またはxref:tomcat-installation-windows.adoc[Windows]） またはTomcatのドキュメントを参照してください。
...
GBIFは、link:https://hub.docker.com/r/gbif/ipt/[Docker Hub]に公開されたDockerイメージを管理しています。このイメージは、DockerコミュニティのTomcat 8.5 / OpenJDK 8 / Debian安定版イメージをもとに構築されています。Tomcatは8080番ポートで公開され、IPTはROOTアプリケーションとして動作します。
